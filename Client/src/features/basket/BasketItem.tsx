...
export default function BasketItem({item } : Props) {
  ...
  return (
    ...
    >
        <Box display='flex' justifyContent='center'>
            ...
            <Box display='flex' flexDirection='column' gap={1}>
                ...
                <Box display='flex' alignItems='center' gap={3}>
                    ...
                    <Typography sx={{fontSize:'1.1rem'}} color="primary"> {currencyFormat(item.price * item.quantity) }</Typography>  
                </Box>
        ...
        color='error' size='small' sx={{border:1, borderRadius:1, minWidth:0, mt:1,mr:1, alignSelf:'start'}}>
            <Close />
         </IconButton>
    </Paper>
  )
